...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  batch_index=0
  labels_index=0
  for i in range(data_index,data_index+batch_size//(skip_window*2)):
    for j in range(i,i+skip_window):
      # print (data[i+skip_window],data[j],data[j+skip_window+1])
      batch[batch_index]=data[i+skip_window]
      batch_index+=1
      labels[labels_index]=data[j]
      labels_index+=1
      batch[batch_index] = data[i + skip_window]
      batch_index+=1
      labels[labels_index]=data[j+skip_window+1]
      labels_index+=1

  data_index=data_index+(batch_size//(skip_window*2))
  # #sw=1,ns=2
  # print (batch)
  # print(labels)
  return batch,labels
